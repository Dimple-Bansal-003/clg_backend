...
public class StudentService {
    ...
    public String saveBulkStudents(@Valid StudentBulkDto dto) {
        ...
        for (StudentJson sj : dto.getStudents()) {
            ...
            if (studentRepo.findByRollNoAndEnrollmentYear(sj.getRollNo(), dto.getEnrollmentYear()).isPresent()) {
                continue;
            }
            ...
            user.setEmail(sj.getEmail().isEmpty() ? sj.getUsername() + "@college.edu" : sj.getEmail());
            ...
            student.setDivision(sj.getDivision() != null ? sj.getDivision() : "A");
            student.setUser(user);
            studentRepo.save(student);

            // Create/get semester for this year
            SemesterEntity sem = semRepo.findBySemesterNoAndAcademicYearId(dto.getSemester(), ay.getId())
                .orElseGet(() -> {
            ...
            sa.setSemester(sem);
            sa.setDivision(sj.getDivision() != null ? sj.getDivision() : "A");
            saRepo.save(sa);

            count++;
        }
        return count + " students enrolled successfully in " + dto.getAcademicYear() + " Semester " + dto.getSemester();
    }
